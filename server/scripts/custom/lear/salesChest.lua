-- salesChest - Release 2.3 - For tes3mp v0.7.0
-- Conceptualized and created by Atkana for tes3mp v0.6.1
-- Updated for tes3mp v 0.7 by Learwolf on 4-11-2021

--[[ INSTALLATION:

** IMPORTANT: THIS FILE REQUIRES ATKANAS itemInfo.lua SCRIPT: https://github.com/Atkana/tes3mp-scripts/blob/master/itemInfo.lua **

1) Save atkanas itemInfo.lua file inside your tes3mp-server/server/scripts/custom folder as:
		itemInfo.lua
...
4) Add the following to customScripts.lua just below where you saved the itemInfo line mentioned above: 
		require("custom.salesChest")

]]

--[[ IMPORTANT NOTES:
	
	* I do not take credit for this script. All credit goes to Atkana. I merely ported it over in a simple-to-install plug and play manner for tes3mp 0.7
	* With the above in mind, I do not provide support for this script. It is an as-is script. Any additional features, functionality, or updates to the sellable 
		items (found in itemInfo.lua) remain up to either Atkana to finish, or anyone else in the community.
	
--]]

--[[ USAGE:
	Use the chat command `/saleschest` in game. A menu will pop up that should guide you through the rest.
--]]
...
config.salesChestGuiId = 31336 --Used internally for the GUI id. If this randomly generated number somehow conflicts with another script's, change this number.
...
function SellChestContents(chest, priceOnly, merchSkill, personality)
	...
	if priceOnly then
		...
		for k, v in pairs(Players) do
			LoadedCells[chest.cell]:LoadContainers(k,LoadedCells[chest.cell].data.objectData,{chest.refIndex})
		end
...
Methods.OnSalesChestCommand = function(pid)
	...
	tes3mp.CustomMessageBox(pid, config.salesChestGuiId, message, 'Sell Items;Assign Container;Close')
end
...
customEventHooks.registerHandler("OnGUIAction", function(eventStatus, pid, idGui, data)
	...
	if isValid ~= false then
		...
		if idGui == config.salesChestGuiId then
			isValid = false
...
Methods.updateSalesChestTimer = function()
	...
	tes3mp.RestartTimer(GlobalSalesChestTimer, time.seconds(1))
end

GlobalSalesChestTimeUpdate = Methods.updateSalesChestTimer
GlobalSalesChestTimer = tes3mp.CreateTimer("GlobalSalesChestTimeUpdate", time.seconds(1))
tes3mp.StartTimer(GlobalSalesChestTimer)


customCommandHooks.registerCommand("saleschest", function(pid, cmd)
	Methods.OnSalesChestCommand(pid)	
end)
